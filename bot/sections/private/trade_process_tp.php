	...
	case 'price':
		...
		if ($sdata['price']) {
			if ($sdata['price'] <= $asset['price']) {
				SendMessage($from_id, td(t('callback_query.trade_tp.condition.errors.price_lower', $user['locale']), [
				...
				SendMessage($from_id, td(t('callback_query.trade_tp.condition.amount', $user['locale']), [
					'symbol' => $asset['symbol'],
					'price' => priceFormat($sdata['price']),
				]), $msg_id);
			}
		} else {
			SendMessage($from_id, t('callback_query.trade_tp.condition.errors.price_invalid', $user['locale']), $msg_id);
		}
	...
	case 'amount':
		...
		if ($sdata['amount']) {
			...
			if (count(getTradeConditionsByStatus($from_id, 0, $mysqli)) < config['TRADE_CONDITIONS_PENDING_MAX']) {
				...
				SendMessage($from_id, td(t('callback_query.trade_tp.condition.success', $user['locale']), [
			...
			SendMessage($from_id, t('callback_query.trade_tp.condition.errors.amount_invalid', $user['locale']), $msg_id);
		}
